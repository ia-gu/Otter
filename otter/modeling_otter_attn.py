...
from flamingo.mpt.modeling_mpt_attn import MPTForCausalLM
...
try:
    print("Trying to import xformers")
    if not XFORMERS_MSG_PRINTED:  # Check if the message has been printed before
        ...
        from xformers_model import LlamaForCausalLM
        from xformers_model.clip_attn import CLIPVisionModel#, LlamaForCausalLM
...
class OtterPerceiverBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, latents: torch.Tensor) -> torch.Tensor:
        ...
        # # 3枚目の画像に対応するアテンションマップを選択し、ヘッドの平均を取る
        # attention_for_third_image = attn[:, :, 2, :, :].mean(axis=1)
        # # アテンションマップを正規化
        # min_val = attention_for_third_image.min()
        # max_val = attention_for_third_image.max()
        # normalized_attention = (attention_for_third_image - min_val) / (max_val - min_val)
        # # 正規化されたアテンションをCPUに移してNumPy配列に変換し、スケール変換
        # normalized_attention_np = normalized_attention.cpu().detach().numpy()
        # normalized_attention_np_scaled = (normalized_attention_np * 255).astype(np.uint8)
        # # 複数のヘッドのアテンションマップの平均を計算
        # average_attention_map = np.mean(normalized_attention_np_scaled, axis=0)  # ヘッド次元に沿って平均

        # # 平均アテンションマップのデータタイプをuint8に変換
        # average_attention_map = average_attention_map.astype(np.uint8)

        # # PILイメージに変換し、適切なリサイズ処理を行う
        # attention_image = Image.fromarray(average_attention_map)
        # original_width = 320
        # original_height = 64
        # target_width = 224
        # scale_factor = target_width / original_width
        # target_height = int(original_height * scale_factor)
        # attention_image_resized = attention_image.resize((target_width, target_height), Image.Resampling.LANCZOS)

        # # アテンションマップの正規化
        # normalized_attention = attention_for_third_image / attention_for_third_image.max()
        # # テンソルをCPUに移し、NumPy配列に変換
        # normalized_attention_np = normalized_attention.cpu().detach().numpy()
        # # 0から255のスケールに変換し、uint8型にキャスト
        # normalized_attention_np_scaled = (normalized_attention_np * 255).astype(np.uint8)
        # attention_np = np.squeeze(normalized_attention_np_scaled)
        # attention_np = attention_np.reshape(-1, 320)
        # attention_image = Image.fromarray(attention_np)
        # # PILイメージに変換
        # # attention_image = Image.fromarray(normalized_attention_np_scaled)
        # # リサイズ
        # # attention_map = attention_image.resize((224, 224))
        # # アスペクト比を保持しながらリサイズするために、リサイズ後の高さを計算
        # original_width = 320
        # original_height = 64
        # target_width = 224
        # scale_factor = target_width / original_width
        # target_height = int(original_height * scale_factor)
        # # 幅を224にリサイズし、高さをアスペクト比に基づいて調整
        # attention_image_resized = attention_image.resize((target_width, target_height), Image.Resampling.LANCZOS)

        # # 224x224にするために必要なパディングを計算
        # padding_top = (224 - target_height) // 2
        # padding_bottom = 224 - target_height - padding_top
        # padding_left = 0
        # padding_right = 0

        # # パディングを追加（黒色でパディング）
        # attention_map = ImageOps.expand(attention_image_resized, border=(padding_left, padding_top, padding_right, padding_bottom), fill=0)
...
class OtterPerceiverResampler(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for block in self.layers:
            latents, attention_map  = block(x, latents)
            attention_maps.append(attention_map)
        # HACK
        average_attention_map = torch.mean(torch.stack(attention_maps, dim=0), dim=0)
        input(average_attention_map.shape)
        # average_attention_map = average_attention_map.squeeze()
        # 最小値と最大値を使用して0から1の範囲に正規化
        min_val = average_attention_map.min()
        max_val = average_attention_map.max()
        normalized_attention = (average_attention_map - min_val) / (max_val - min_val)

        # CPUに移動し、NumPy配列に変換
        normalized_attention_np = normalized_attention.cpu().detach().numpy()
        # 0から255のスケールに変換し、uint8型にキャスト，3枚目の画像の情報をキャッチ
        attention_map_for_third_image = (normalized_attention_np * 255).astype(np.uint8)[2]
        # 各ヘッドの平均を計算
        average_attention_map = np.mean(attention_map_for_third_image, axis=0)
        # 平均アテンションマップをさらにRGBチャンネルにわたって平均化して、グレースケールに変換
        # ここで、時間的なシーケンスまたはRGBチャンネルの概念があればの話
        average_attention_map = np.mean(average_attention_map, axis=0)
        # アテンションマップの値を0から1の範囲に正規化
        normalized_attention_map = average_attention_map / np.max(average_attention_map)
        # PILを使用して画像として保存する場合
        # 注意: PIL.Image.fromarrayは0から255の整数値を期待するため、スケーリングが必要(64*320)
        normalized_attention_map = (normalized_attention_map * 255).astype(np.uint8)
        attention_map = Image.fromarray(normalized_attention_map)
        attention_map = attention_map.resize((224, 224), Image.Resampling.LANCZOS)
        # attention_image = attention_image.resize((224, 224), Image.Resampling.LANCZOS)
        # import pdb; pdb.set_trace()
        return self.norm(latents), [attention_map]
...
class OtterGatedCrossAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        # media = output of PerceiverResampler, (num_beams: 3, ICL: 3, patch: 64, dimension: 1024)
        # x = lang feature, (num_beams: 3, input_token_length: 145, dimension: 4096)
        # attention, (num_beams: 3, input_token_length: 145, dimension: 4096)
        attention = self.attn(x, media, media_locations=media_locations, attend_previous=attend_previous)
        x = (
            self.attn(
                x,
                media,
...
class OtterForConditionalGeneration(OtterPreTrainedModel):
    ...
    def _encode_vision_x(self, vision_x: torch.Tensor):
        ...
        with torch.no_grad():
            vision_x = self.vision_encoder(vision_x)[0][:, 1:, :]
        vision_x = rearrange(vision_x, "(b T F) v d -> b T F v d", b=b, T=T, F=F)
        vision_x, attention_maps = self.perceiver(vision_x)  # reshapes to (b, T, n, d)
    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        # torch.Size([3, 3, 1, 3, 224, 224])
        attention_maps = self._encode_vision_x(vision_x=vision_x)
